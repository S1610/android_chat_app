...
class _ClassWidgetState extends State<ClassWidget> {
  ...
  void onEditPressed() {
    showDialog(
      context: context,
      builder: (context) {
        TextEditingController classNameController = TextEditingController(text: widget.className);
        return AlertDialog(
          ...
          actions: [
            ...
            TextButton(
              onPressed: () {
                setState(() {
                  widget.className = classNameController.text; // Update the class name
                });
                Navigator.of(context).pop();
              },
    ...
    if (Platform.isAndroid) {
      ...
      if (status.isGranted) {
        ...
        if (status.isGranted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Storage permission denied. Please grant permission to save files.')),
          );
          return false;
        }
      }
  ...
  Future<void> _saveImagesToClassDirectory(List<XFile> images, String className, String? parentDirectoryPath) async {
    // if (!await _requestPermissions()) {
    //   ScaffoldMessenger.of(context).showSnackBar(
    ...
    try {
      ...
      if (parentDirectoryPath != null && parentDirectoryPath.isNotEmpty) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('No custom directory selected. Saving to app documents.')),
        );
      }
      ...
      int savedCount = 0;
      for (int i = 0; i < images.length; i++) { // Use a loop with index
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Saved $savedCount images to: $classDirectoryPath')),
      );
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
                    ...
                    PopupMenuButton<String>(
                      onSelected: (value) async {
                        if (value == 'delete') {
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(content: Text('${widget.className} deleted')),
                          );
                        } else if (value == 'disable') {
                          setState(() {
                            widget.classEnabled = false;
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(content: Text('${widget.className} disabled')),
                            );
                          });
                          ...
                          setState(() {
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(content: Text('${widget.className} enabled')),
                            );
                          });
                          ...
                          if (selectedDir != null) {
                            await _saveImagesToClassDirectory(_selectedImages, widget.className, selectedDir);
                          } else {
                            ScaffoldMessenger.of(context).showSnackBar(
