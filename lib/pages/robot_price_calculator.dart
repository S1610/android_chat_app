...
import 'package:android_chat_app/states/totals_notifier.dart'; // For TotalsNotifier and CardInvoiceData
import 'package:android_chat_app/widgets/robot_price_widgets.dart'; // For Product and ProductLoader
import 'package:android_chat_app/services/robot_json_handler.dart'; // For CsvProductLoader and ProductDataFromCsv
import 'package:android_chat_app/widgets/invoice_slip_widget.dart'; // For InvoiceSlipWidget
...
class _RobotPriceCalculatorViewState extends State<RobotPriceCalculatorView> {
  ...
  //final String jsonAssetPath = 'assets/robot_products.json'; // Define your JSON path

  @override 
  void initState() {
    super.initState();
    // Load product data from JSON when the widget is initialized
    _productsFuture = _loader.loadProductsFromFirestore();
  }
    ...
    return Scaffold(
      body: RefreshIndicator(
        onRefresh: () async {
          ...
          await _productsFuture; // Wait for the data to be reloaded
        },
          ...
          builder: (context, snapshot) {
            ...
            // Handle no data or empty data state
              ...
              return Center(
                child: Padding(
                  padding: const EdgeInsets.all(16.0),
                  child: Text("No products found. Please check the file content."),
                )
              );
            }
